...
SIZEKB=$(( $SIZE/1000 )) 
echo "Filesize: ${SIZEKB}Kb"
#по пунктам:  1. командою read зчитую назву файла в зміну FILE.  Потім за допомогою команди head -c 4KB /dev/urandom > $FILE   створюю  файл.  Проходжусь циклом поки він не досягне 1024кб. ,  кожну ітерацію збільшуючи  на поточний розмір.  В кожній ітерації виводжу значення розміру файлу.    {}